...
import type { Folder, MinimalFolder } from './types/folder';
import type { TreeItem, TreeItem1 } from './types';
...
export function buildTree1(folders: MinimalFolder[], parentFolderID: string | null): TreeItem1[] {
	const currentFolders = folders.filter((folder) => folder.subfolder_of === parentFolderID);

	return currentFolders.map((folder) => {
		const children = buildTree1(folders, folder.folder_id);
...
export function buildTree2(folders: MinimalFolder[], parentFolderID: string | null): TreeItem1[] {
	const currentFolders = folders.filter((folder) => folder.subfolder_of === parentFolderID);
	// @ts-ignore: Ignore TypeScript for this function
	return currentFolders.map((folder) => {
		const children = buildTree2(folders, folder.folder_id);
